    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <p className="text-[10px] text-slate-550 dark:text-slate-400 mt-1 font-medium">Claim packages in your designated delivery zone to start delivering.</p>
        </div>
      ...
      {filteredAvailableOrders.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800/80 rounded-2xl p-4 shadow-sm flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl bg-blue-500/10 dark:bg-blue-500/20 text-blue-550 dark:text-blue-400 flex items-center justify-center shrink-0">
              <DollarSign size={16} />
            </div>
            <div>
      ...
      {/* Cards List */}
      {filteredAvailableOrders.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111827] py-16 text-center text-xs text-slate-505 shadow-sm flex flex-col items-center justify-center gap-3">
          ...
          <div>
            <p className="font-extrabold text-slate-705 dark:text-slate-350 text-sm">Pool is fully claimed</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {filteredAvailableOrders.map((order) => (
            <div 
              key={order._id} 
              className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5.5 shadow-sm flex flex-col justify-between gap-5 hover:shadow-lg hover:border-slate-300 dark:hover:border-slate-700/80 transition-all duration-300 group"
            >
              <div className="space-y-4">
                <div className="flex justify-between items-start gap-3">
                  ...
                  <span className={`text-[9px] font-black border px-2.5 py-1 rounded-lg uppercase tracking-wider ${
                ...
                <div className="text-xs space-y-3">
                  <div className="flex items-start gap-2.5">
                    <div className="h-6 w-6 rounded bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-850 flex items-center justify-center shrink-0 text-slate-400 dark:text-slate-500">
                      <MapPin size={12} />
                    </div>
                    <div>
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-slate-650 dark:text-slate-400 font-bold truncate mt-0.5">
              ...
              <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-850/80 pt-4 mt-1">
                <div className="flex flex-col">
                  <span className="text-[8px] font-black text-slate-450 dark:text-slate-500 uppercase tracking-widest">Payout Commission</span>
                  <span className="font-black text-slate-900 dark:text-white text-base">₹{order.amount}</span>
                </div>
                <button
                  onClick={() => claimOrderHandler(order._id)}
                  className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl px-4 py-2.5 text-xs font-black uppercase tracking-wider transition-all duration-300 hover:shadow-md hover:shadow-blue-500/10 active:scale-95 cursor-pointer flex items-center gap-2 group-hover:scale-105"
                >
