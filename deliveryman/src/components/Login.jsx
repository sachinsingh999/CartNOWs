...
const Login = ({ setToken, setDriver }) => {
  ...
  return (
    <div className="h-screen w-screen grid lg:grid-cols-[1.1fr_1fr] bg-[#F9FAFB] dark:bg-[#070A13] text-slate-800 dark:text-slate-100 font-sans transition-colors duration-200 overflow-hidden">
      ...
      <div className="relative hidden lg:flex flex-col justify-between p-8 overflow-hidden h-full">
        ...
        <div className="relative z-20 mt-auto text-left max-w-xl text-white">
          ...
          <p className="mt-3 text-xs leading-relaxed text-slate-305 text-slate-300 font-light">
      ...
      <div className="flex flex-col justify-center items-center px-6 py-6 md:px-12 relative h-full overflow-y-auto">
        ...
        <button
          onClick={() => navigate("/")}
          className="lg:hidden absolute top-5 left-5 flex items-center gap-1.5 text-xs font-bold text-slate-505 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition cursor-pointer"
        >
          <ArrowLeft size={14} />
          <span>Back</span>
        </button>
        ...
        <div className="w-full max-w-sm space-y-6">
          ...
          <div className="flex flex-col items-center text-center space-y-2">
            <div className="h-10 w-10 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#070A13] flex items-center justify-center shadow-sm">
              <Logo variant="icon" className="h-6 w-6 text-slate-850 dark:text-white" />
            </div>
            <div>
          ...
          <form onSubmit={onSubmitHandler} className="space-y-3.5">
            <div>
              <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-550 dark:text-slate-455">Email Address</label>
            ...
            <div>
              <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-550 dark:text-slate-455">Password</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3.5 text-slate-400 dark:text-slate-500 pointer-events-none">
                  <Lock size={14} />
                </span>
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-955 dark:bg-slate-950/40 text-xs text-slate-850 dark:text-white outline-none transition duration-200 placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:bg-white dark:focus:bg-slate-950/80 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10"
                  required
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute inset-y-0 right-0 flex items-center pr-3.5 text-slate-400 hover:text-slate-700 dark:text-slate-550 dark:hover:text-white cursor-pointer"
                >
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white py-3 text-xs font-black uppercase tracking-wider transition duration-300 shadow-md hover:shadow-lg hover:shadow-blue-500/20 active:scale-[0.98] cursor-pointer mt-4 disabled:opacity-50"
            >
          ...
          <div className="space-y-3 pt-5 border-t border-slate-200 dark:border-slate-900">
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-900">
              ...
              <div className="grid grid-cols-2 gap-3">
                <a
                  ...
                  className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 text-[10px] font-bold text-slate-655 text-slate-600 dark:text-slate-400 hover:border-orange-500 dark:hover:border-orange-500 hover:bg-orange-50/35 dark:hover:bg-orange-950/20 hover:text-orange-600 dark:hover:text-orange-400 transition-all duration-300 transform hover:-translate-y-0.5 active:scale-95 shadow-sm"
                >
            ...
            <div className="flex items-center justify-center gap-2 text-[10px] text-slate-450 dark:text-slate-500 pt-3 border-t border-slate-200 dark:border-slate-800">
