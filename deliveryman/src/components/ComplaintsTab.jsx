      ...
      <form onSubmit={handleComplaintSubmit} className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4.5 hover:shadow-md transition-all duration-300">
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            className="w-full border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-xs font-bold outline-none bg-slate-50 dark:bg-slate-900 transition text-slate-900 dark:text-white cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
          >
        ...
        <div className="space-y-1.5">
          <label className="text-[9px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Subject</label>
          <input
            type="text"
            placeholder="Summarize the issue..."
            value={complaintForm.subject}
            onChange={(e) => setComplaintForm(c => ({ ...c, subject: e.target.value }))}
            className="w-full border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-xs font-bold outline-none bg-slate-50 dark:bg-slate-900 transition text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
          />
        </div>

        <div className="space-y-1.5">
          <label className="text-[9px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">Description</label>
          <textarea
            rows="4"
            placeholder="Detail what happened..."
            value={complaintForm.description}
            onChange={(e) => setComplaintForm(c => ({ ...c, description: e.target.value }))}
            className="w-full border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-xs outline-none bg-slate-50 dark:bg-slate-900 transition resize-none font-semibold text-slate-900 dark:text-white leading-relaxed focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
          />
        </div>

        <button
          type="submit"
          className="w-full bg-blue-600 hover:bg-blue-700 text-slate-100 dark:text-white rounded-xl py-3 text-xs font-black uppercase tracking-wider transition-all duration-200 shadow-md shadow-blue-500/10 active:scale-95 cursor-pointer flex items-center justify-center gap-1.5"
        >
          <PlusCircle size={12} />
