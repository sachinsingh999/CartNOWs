            ...
            <div key={task._id} className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5 shadow-sm space-y-4 hover:shadow-md transition-all duration-300">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {task.returnType === "Exchange" && task.exchangeSize && (
                      <span className="bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-800 rounded-md px-2 py-0.5 text-[9px] font-black uppercase tracking-wider">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {task.status === "Completed" ? (
                    ...
                    <select
                      ...
                      className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-2.5 py-1.5 text-xs font-bold text-slate-900 dark:text-white outline-none cursor-pointer transition focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
                    >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-xs text-slate-700 dark:text-slate-300">
                ...
                <div className="space-y-2 bg-slate-50/50 dark:bg-slate-900/40 p-4 rounded-xl border border-slate-200/60 dark:border-slate-800/80">
                  <p className="font-black text-[9px] uppercase tracking-widest text-slate-400 dark:text-slate-500">Returned Item</p>
                  <p className="font-black text-slate-900 dark:text-white text-sm">{task.itemName}</p>
                  <p className="text-slate-500 dark:text-slate-400 font-semibold">Size: {task.itemSize || "—"} · Qty: {task.quantity}</p>
                  <p className="font-black text-slate-900 dark:text-slate-200 mt-2 text-sm">Value: ₹{task.amount}</p>
                </div>

                {/* Customer Address */}
                <div className="space-y-2 bg-slate-50/50 dark:bg-slate-900/40 p-4 rounded-xl border border-slate-200/60 dark:border-slate-800/80">
                  <p className="font-black text-[9px] uppercase tracking-widest text-slate-400 dark:text-slate-500">Customer Address</p>
                  {task.orderAddress ? (
                    <>
                ...
                {/* Instructions */}
                <div className="space-y-2 bg-slate-50/50 dark:bg-slate-900/40 p-4 rounded-xl border border-slate-200/60 dark:border-slate-800/80">
                  <p className="font-black text-[9px] uppercase tracking-widest text-slate-400 dark:text-slate-500">Task Notes</p>
                  <p className="text-slate-700 dark:text-slate-300 leading-relaxed font-semibold">
                    <span className="font-black text-slate-400 dark:text-slate-500 block text-[9px] uppercase tracking-wider mb-0.5">Reason:</span> {task.reason}
                  </p>
                  {task.adminNote && (
