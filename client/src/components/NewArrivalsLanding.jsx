...
import heroImg from "../assets/new_arrivals_hero.webp";
...
const NewArrivalsLanding = ({
  ...
  useEffect(() => {
    cachedGet(`${backendUrl}/api/promo-banners/active?placement=new_arrivals_hero`, {}, 60000)
      .then((res) => {
        if (res.data?.success) {
      ...
      .catch((err) => {
        console.warn("Could not load dynamic new arrivals hero banners:", err);
      });
  }, []);
  ...
  // Live Drop Countdown Timer (7 Days campaign cycle)
  ...
  return (
    <div className="min-h-screen bg-slate-50/70 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300 pb-20 font-sans">
      ...
      <div className="w-full px-3 sm:px-6 lg:px-8 pt-2.5">
        ...
        {/* 1. HERO SECTION SPLIT IN TWO HALVES (50% LEFT WHITISH-YELLOWISH TEXT / 50% RIGHT FULL IMAGE VIEWER) */}
        <div 
          ...
          <div className="w-full lg:w-1/2 p-6 sm:p-10 lg:p-12 flex flex-col justify-center relative z-10 bg-[#FAF7EE] dark:bg-stone-950 text-slate-900 dark:text-white transition-colors duration-300">
            ...
            <AnimatePresence mode="wait" custom={direction}>
              ...
              >
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-sm bg-amber-500/15 border border-amber-500/35 text-amber-900 dark:text-amber-300 text-xs font-bold tracking-wider w-fit mb-3">
                  <span>{currentBanner?.tagline || "JUST RELEASED"}</span>
                </div>
                ...
                <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-black uppercase tracking-tight text-slate-900 dark:text-white leading-[0.95] mb-3">
                  {currentBanner?.title ? (
                    ...
                    <>
                      NEW <br />
                      <span className="text-amber-600 dark:text-amber-400">ARRIVALS</span>
                    </>
                  )}
                </h1>

                <p className="text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300 leading-relaxed max-w-md">
                  {currentBanner?.subtitle || "Fresh styles & modern vibes. Explore handpicked weekly drops straight from verified designer catalogs."}
                </p>
              </motion.div>
            </AnimatePresence>

            {/* LIVE DROP COUNTDOWN TIMER GLASS CARD */}
            <div className="mt-5 p-3.5 rounded-sm bg-white/90 dark:bg-slate-900/90 border border-amber-200/80 dark:border-slate-800 max-w-xs sm:max-w-sm flex items-center justify-between shadow-xs">
              <div>
                <span className="text-[10px] font-black uppercase tracking-widest text-amber-800 dark:text-amber-400 block mb-1">
                  {currentBanner?.discountTag || "NEXT CAPSULE DROP IN"}
                </span>
                <div className="flex items-center gap-1.5 text-xs sm:text-base font-black font-mono text-slate-900 dark:text-white">
                  ...
                  <span>:</span>
                  <span className="bg-amber-500 text-slate-950 px-2 py-0.5 rounded-xs">{String(timeLeft.seconds).padStart(2, '0')}s</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 flex items-center gap-3">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => {
                  if (currentBanner?.linkUrl && currentBanner.linkUrl !== "/catalog/collection/new-arrivals") {
              ...
              >
                <span>{currentBanner?.ctaText || "Explore Drops"}</span>
            ...
            <div className="hidden sm:flex absolute bottom-6 right-6 sm:bottom-8 sm:right-8 w-26 h-26 sm:w-30 sm:h-30 rounded-full bg-slate-950/85 backdrop-blur-md border border-slate-700/60 items-center justify-center shadow-2xl hover:scale-105 transition-transform cursor-pointer z-20">
              <div className="relative w-full h-full flex items-center justify-center">
                <svg className="w-full h-full animate-[spin_20s_linear_infinite]" viewBox="0 0 100 100">
                  <path id="sharpStampPath" d="M 50, 50 m -37, 0 a 37,37 0 1,1 74,0 a 37,37 0 1,1 -74,0" fill="none" />
                  <text className="text-[8px] font-bold uppercase tracking-[0.24em] fill-white">
                    <textPath href="#sharpStampPath" startOffset="0%">
        ...
        {/* 2. FRAMELESS CATEGORY TICKER WITH SMOOTH DROP ANIMATION */}
        <div className="mb-5 space-y-2">
          ...
          {/* INFINITE MARQUEE TICKER ANIMATING WITH TOP-TO-BOTTOM SLIDE ENTRANCE */}
            ...
            >
              {duplicatedCategories.map((cat, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Full Box Cover Background Image */}
                    <img 
                      src={cat.image} 
                      alt={cat.label} 
        ...
        <div id="arrivals-grid" className="sticky top-14 sm:top-16 z-30 w-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 rounded-sm p-2.5 sm:p-3 mb-6 shadow-md transition-all">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-6 border border-slate-200/80 dark:border-slate-800 rounded-sm bg-white/80 dark:bg-slate-900/80 backdrop-blur-md text-center animate-fade-in my-6 shadow-xs">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">No New Drops Match Your Filters</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mt-1 leading-normal">
              We couldn't find any products matching your active category or filter settings.
            </p>
            <button
              onClick={() => {
            ...
            {totalPages > 1 && (
              <div className="mt-10 pt-5 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
                <p className="text-xs font-medium text-slate-600 dark:text-slate-400">
                  ...
                  <span className="font-bold text-slate-900 dark:text-white">{processedProducts.length}</span> new products
                </p>
...
export default NewArrivalsLanding;
