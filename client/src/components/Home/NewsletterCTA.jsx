...
const NewsletterCTA = () => {
  ...
  return (
    <section className="w-full px-6 sm:px-12 lg:px-20 py-6 select-none">
      <div className="relative rounded-[40px] overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-700 to-violet-900 text-white p-8 sm:p-12 lg:p-16 text-center shadow-xl border border-white/10 flex flex-col items-center gap-4">
        ...
        <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-3 w-full max-w-md mt-4 select-none">
          <input
            ...
            required
            className="w-full px-5 py-3.5 bg-white/10 border border-white/25 rounded-2xl text-xs sm:text-sm outline-none placeholder-white/55 font-bold focus:border-white focus:bg-white/15 text-white"
          />
          <button
            type="submit"
            className="px-6 py-3.5 bg-white hover:bg-slate-50 text-indigo-950 font-black uppercase text-xs tracking-wider rounded-2xl transition-all shadow-lg shrink-0 cursor-pointer active:scale-95 border-none"
          >
            Subscribe
          </button>
        </form>
