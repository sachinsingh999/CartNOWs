      ...
      <div className="relative rounded-none overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-700 to-violet-900 text-slate-100 dark:text-white p-8 sm:p-12 lg:p-16 text-center shadow-xl border border-white/10 flex flex-col items-center gap-4">
        ...
        <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-3 w-full max-w-md mt-4 select-none">
          <input
            ...
            required
            className="w-full px-5 py-3.5 bg-white/10 border border-white/25 rounded-none text-xs sm:text-sm outline-none placeholder-white/55 font-bold focus:bg-white/15 text-slate-100 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
          />
          <button
            type="submit"
            className="px-6 py-3.5 bg-white dark:bg-slate-900 hover:bg-slate-50 text-indigo-950 font-black uppercase text-xs tracking-wider rounded-none transition-all shadow-lg shrink-0 cursor-pointer active:scale-95 border-none"
          >
            Subscribe
          </button>
        </form>
