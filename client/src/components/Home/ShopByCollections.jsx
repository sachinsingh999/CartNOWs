...
const ShopByCollections = ({ trendingCollections = [] }) => {
  ...
  return (
    <section className="w-full px-6 sm:px-12 lg:px-20 mb-10 select-none text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {collections.map((col) => (
          <div
            key={col.id}
            className={`group rounded-[28px] border p-4 flex flex-col justify-between transition-all duration-300 hover:shadow-[0_20px_40px_rgba(0,0,0,0.03)] hover:-translate-y-1 ${col.colorClass}`}
          >
            ...
            <div className="flex flex-wrap gap-1.5 mb-3.5">
              {col.tags.map((tag) => (
                <span
                  key={tag.name}
                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-extrabold border transition-colors cursor-pointer ${col.pillsColor}`}
                >
            ...
            <div className="relative rounded-2xl overflow-hidden aspect-[16/10] bg-white/50 dark:bg-slate-950/20 p-1.5 mb-4 flex items-center justify-center border border-white/40 dark:border-slate-800/40 shadow-inner">
              <img
                src={col.image}
                alt={col.title}
                loading="lazy"
                className="max-h-[95%] max-w-[95%] object-contain rounded-xl transition-transform duration-500 group-hover:scale-105"
              />
              {col.overlayBadge && (
