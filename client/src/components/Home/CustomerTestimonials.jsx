      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4">
        ...
        <div className="flex gap-2 shrink-0">
          ...
          <button
            ...
            className="h-9 w-9 rounded-full border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center justify-center text-slate-605 dark:text-slate-400 cursor-pointer transition shadow-sm"
          >
              ...
              <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[28px] p-6 shadow-xs flex flex-col justify-between text-left h-[200px]">
                <div>
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-full overflow-hidden border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 shrink-0">
                      <img src={item.avt} alt={item.name} className="w-full h-full object-cover" />
                    </div>
                    <div className="leading-none">
                      <h4 className="font-extrabold text-[13px] text-slate-850 dark:text-slate-200">{item.name}</h4>
                  ...
                  <div className="flex gap-0.5 text-amber-500 my-3">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} size={11} className="fill-amber-500 stroke-none" />
                    ))}
                  </div>

                  <p className="text-[11.5px] text-slate-550 dark:text-slate-400 font-semibold leading-relaxed line-clamp-3">
                    "{item.review}"
                  </p>
                </div>

                <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 mt-auto flex items-center justify-between text-[8.5px] font-black tracking-wider text-emerald-600 dark:text-emerald-450 uppercase">
