...
const HomeHero = ({ onShowDealOfDay, hasActiveDeal }) => {
  ...
  // Horizontal slide variants (Direction > 0 means next/right, Direction < 0 means prev/left)
  ...
  const modelSlideVariants = useMemo(() => ({
    initial: (dir) => ({
      x: dir > 0 ? "100%" : "-100%",
      opacity: 0,
                  ...
                  {customBanners[slideIdx] && (() => {
                    ...
                    return (
                      <motion.div
                        ...
                        animate="animate"
                        exit="exit"
                        className={`absolute inset-0 w-full h-full flex flex-row items-center text-slate-100 dark:text-white overflow-hidden ${banner.backgroundTheme || 'bg-gradient-to-r from-slate-900 to-indigo-950'}`}
                      >
          ...
          `}</style>
            ...
            <div className="w-full px-4 sm:px-8 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center z-10 relative">
              ...
              <div className="flex flex-col space-y-5 lg:space-y-6 text-left max-w-xl mx-auto lg:mx-0 items-start z-10 relative">
                ...
                <div className="h-[200px] sm:h-[220px] lg:h-[240px] w-full relative">
                  <AnimatePresence mode="wait">
                    <motion.div
                      key={slideIdx}
                    ...
                    >
                      {/* Staggered Rating Header */}
                      <motion.div variants={textItemVariants} className="flex items-center gap-2 flex-wrap select-none mb-0.5">
