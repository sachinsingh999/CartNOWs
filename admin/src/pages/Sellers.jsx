              ...
              >
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase tracking-wider border ${ seller.status === "active" ? "bg-emerald-50 text-emerald-600 border-emerald-200/50 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20" : seller.status === "pending" ? "bg-amber-50 text-amber-600 border-amber-200/50 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20" : "bg-rose-50 text-rose-600 border-rose-200/50 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20" }`}>
                      {seller.status}
                    </span>
                  </div>
              ...
              <div className="space-y-3 shrink-0">
                ...
                <div className="flex flex-col gap-2.5">
                  {selectedSeller.status === "pending" && (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-100 dark:text-white text-[11px] uppercase tracking-wider font-bold transition shadow-xs cursor-pointer active:scale-95"
                      >
                        Approve
                      </button>
                      <button
                        onClick={() => handleStatusChange(selectedSeller._id, "suspended")}
                        className="flex-1 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-600 text-slate-100 dark:text-white text-[11px] uppercase tracking-wider font-bold transition shadow-xs cursor-pointer active:scale-95"
                      >
                        Reject
                      </button>
                    </div>
                  ...
                  {selectedSeller.status === "suspended" && (
                    <button
                      onClick={() => handleStatusChange(selectedSeller._id, "active")}
                      className="w-full py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-100 dark:text-white text-[11px] uppercase tracking-wider font-bold transition shadow-xs cursor-pointer active:scale-95"
                    >
                      Re-activate Account
                    </button>
                  )}
              ...
              <form onSubmit={handleCommissionUpdate} className="space-y-3 border-t border-slate-100 dark:border-slate-800 pt-5 shrink-0">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-white text-xs font-semibold outline-none dark: transition focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
                  />
                  <button
                    type="submit"
                    className="px-4 py-2.5 bg-slate-900 dark:bg-slate-800 text-slate-100 dark:text-white dark:text-white rounded-xl text-[11px] uppercase tracking-wider font-bold hover:bg-slate-800 dark:hover:bg-slate-700 transition cursor-pointer shadow-xs active:scale-95"
                  >
                    Update
                  </button>
                </div>
                      ...
                      {selectedSeller.payoutRequests.map((req) => (
                        <div key={req._id} className="border border-slate-200 dark:border-slate-700/80 rounded-xl p-3.5 space-y-3 hover:border-slate-300 dark:hover:border-slate-600 transition bg-white dark:bg-slate-800/30 shadow-xs">
                          <div className="flex justify-between items-center text-xs">
                            ...
                            <span className={`px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase tracking-wider border ${ req.status === "approved" ? "bg-emerald-50 text-emerald-600 border-emerald-200/50 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20" : req.status === "pending" ? "bg-amber-50 text-amber-600 border-amber-200/50 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20" : "bg-rose-50 text-rose-600 border-rose-200/50 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20" }`}>
                              {req.status}
                            </span>
                          </div>
                          {req.status === "pending" && (
                            <div className="flex gap-2">
                              <button
                                ...
                                className="flex-1 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-slate-100 dark:text-white text-[10px] uppercase tracking-wider font-bold transition shadow-xs cursor-pointer active:scale-95"
                              >
                                Approve
                              </button>
                              <button
                                onClick={() => handlePayoutStatus(req._id, "rejected")}
                                className="flex-1 py-1.5 rounded-lg bg-rose-500 hover:bg-rose-600 text-slate-100 dark:text-white text-[10px] uppercase tracking-wider font-bold transition shadow-xs cursor-pointer active:scale-95"
                              >
                                Decline
                              </button>
                            </div>
