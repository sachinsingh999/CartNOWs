...
const Sidebar = ({ isCollapsed, isMobileOpen, setIsMobileOpen }) => {
  ...
  return (
    <aside 
      className={`bg-white dark:bg-[#111827] border-r border-slate-200 dark:border-white/[0.08] p-3 flex flex-col justify-between overflow-y-auto custom-scrollbar shrink-0 transition-all duration-300
        /* Desktop stickiness */
        md:sticky md:top-0 md:z-auto
        ${isCollapsed ? "md:w-[64px]" : "md:w-[210px]"}
        
        /* Mobile sliding drawer overlay */
        fixed top-[60px] bottom-0 left-0 w-[240px] z-45
        ...
        {groups.map((group, groupIdx) => (
          <div key={groupIdx} className="space-y-1.5 flex flex-col">
            ...
            {(!isCollapsed || window.innerWidth < 768) && (
              <p className="text-[10px] font-bold text-slate-400 dark:text-slate-505 uppercase tracking-widest px-3 mb-1 shrink-0">
                {group.title}
              </p>
            )}
